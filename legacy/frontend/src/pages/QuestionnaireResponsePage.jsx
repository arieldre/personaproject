...
const QuestionnaireResponsePage = () => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50">
        <div className="text-center">
          ...
          <p className="mt-3 text-slate-600">Loading assessment...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4">
        <div className="max-w-md w-full text-center">
          <div className="w-20 h-20 mx-auto mb-6 rounded-2xl bg-red-50 flex items-center justify-center shadow-sm border border-red-100">
            <AlertCircle className="w-10 h-10 text-red-500" />
          </div>
          <h1 className="text-2xl font-bold text-slate-900 mb-3">Questionnaire Not Found</h1>
          <p className="text-slate-600">This questionnaire may have been closed or doesn't exist.</p>
        </div>
      </div>
    );
  }
  ...
  if (submitted) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4">
        <div className="max-w-md w-full text-center">
          <div className="w-24 h-24 mx-auto mb-6 rounded-full bg-green-50 flex items-center justify-center shadow-sm border border-green-100">
            <CheckCircle className="w-12 h-12 text-green-600" />
          </div>
          <h1 className="text-3xl font-bold text-slate-900 mb-3">Thank You!</h1>
          <p className="text-slate-600 mb-8">Your response has been submitted successfully.</p>
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
            <Sparkles className="w-8 h-8 text-indigo-600 mx-auto mb-3" />
            <p className="text-slate-700">Your answers will be processed using VCPQ vector analysis to generate accurate AI personas.</p>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 mb-4 hover:shadow-md transition-shadow">
        <div className="flex items-start gap-4 mb-5">
          <span className="w-10 h-10 rounded-xl bg-indigo-50 text-indigo-700 text-sm font-bold flex items-center justify-center flex-shrink-0 border border-indigo-100">
            A{question.id}
          </span>
          <div className="flex-1">
            <p className="font-medium text-slate-900 text-lg leading-relaxed">{question.question}</p>
            ...
            return (
              <button
                ...
                className={`p-4 rounded-xl text-center transition-all duration-200 ${isSelected
                    ...
                    : 'bg-slate-50 text-slate-600 hover:bg-slate-100 border border-slate-200 hover:border-indigo-200'
                  }`}
              >
                <div className={`text-2xl font-bold mb-1 ${isSelected ? 'text-white' : 'text-slate-400'}`}>
                  {numericValue}
                </div>
  ...
  const GenericQuestion = ({ question }) => (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 mb-4 hover:shadow-md transition-shadow">
      <div className="flex items-start gap-3 mb-4">
        <span className="w-8 h-8 rounded-lg bg-slate-100 text-slate-700 text-sm font-medium flex items-center justify-center">
          {question.id}
        </span>
        <div className="flex-1">
        ...
        <div className="space-y-2">
          {question.options?.map(option => (
            ...
            >
              ...
              <span className={`w-5 h-5 rounded-full border-2 mr-3 flex items-center justify-center ${answers[question.id] === option ? 'border-indigo-500' : 'border-slate-300'
                }`}>
      ...
      {question.type === 'multiple_choice' && (
        <div className="space-y-2">
          {question.options?.map(option => {
            ...
            return (
              <label key={option} className={`flex items-center p-4 rounded-xl border-2 cursor-pointer transition-all ${selected ? 'border-indigo-500 bg-indigo-50' : 'border-slate-200 hover:border-indigo-300 bg-white'
                }`}>
                <input type="checkbox" checked={selected} onChange={() => handleMultiSelect(question.id, option)} className="sr-only" />
                <span className={`w-5 h-5 rounded border-2 mr-3 flex items-center justify-center ${selected ? 'border-indigo-500 bg-indigo-500' : 'border-slate-300'
                  }`}>
      ...
      {question.type === 'text' && (
        <textarea value={answers[question.id] || ''} onChange={(e) => handleAnswer(question.id, e.target.value)}
          ...
          className="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent resize-none bg-slate-50" />
      )}
    </div>
  );
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="text-center mb-8">
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <span className="text-xl font-bold text-slate-700">VCPQ Assessment</span>
          </div>
          <h1 className="text-3xl font-bold text-slate-900">{questionnaire.name}</h1>
          {questionnaire.description && <p className="text-slate-600 mt-2 max-w-xl mx-auto">{questionnaire.description}</p>}
        </div>
        ...
        {hasModules && (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4 mb-6">
            <div className="flex items-center justify-between">
              ...
              <div className="text-center">
                <p className="font-semibold text-slate-900 text-lg">Module: {modules[currentModule]}</p>
                <p className="text-sm text-slate-500">{currentModule + 1} of {modules.length}</p>
              </div>
              <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent bg-slate-50"
                />
              </div>
              <div>
        ...
        {hasModules && currentModule < modules.length - 1 ? (
          <div className="mt-8">
            <button
              ...
              className="w-full py-4 px-6 bg-indigo-600 text-white font-semibold rounded-xl hover:bg-indigo-700 transition-all shadow-lg hover:shadow-xl flex items-center justify-center gap-2 text-lg"
            >
          ...
          <div className="mt-8">
            <button
              ...
              disabled={submitting}
              className="w-full py-4 px-6 bg-emerald-600 text-white font-semibold rounded-xl hover:bg-emerald-700 transition-all shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 text-lg"
            >
              {submitting ? <><Loader2 className="w-5 h-5 animate-spin" />Submitting...</> : 'Submit Response'}
            </button>
            <p className="text-center text-sm text-slate-500 mt-4">Your responses will be used for VCPQ vector analysis.</p>
          </div>
        )}
      </div>
    </div>
