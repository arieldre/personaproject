...
/**
 * DELETE /api/questionnaires/:id/personas
 * Delete all personas for a questionnaire
 */
router.delete('/:id/personas', authenticate, validateUUIDParams('id'), requireAdminAccess, async (req, res) => {
  try {
    const questionnaire = await query('SELECT * FROM questionnaires WHERE id = $1', [req.params.id]);
    if (!questionnaire.rows[0]) {
      return res.status(404).json({ error: 'Questionnaire not found' });
    ...
    // Delete all personas for this questionnaire
    const result = await query('DELETE FROM personas WHERE questionnaire_id = $1', [req.params.id]);
    ...
    res.json({ message: 'All personas deleted', count: result.rowCount });
  } catch (error) {
    console.error('Delete questionnaire personas error:', error);
